...
func TestStorageWriteErrorAndUnsupportedOperations(t *testing.T) {
	...
	err := backend.Save(t.Context(), driver.Record{ID: "trace", Data: []byte(`{}`)}, driver.SaveOptions{})
...
func TestStorageRejectsInvalidState(t *testing.T) {
	...
	if err := backend.Save(t.Context(), driver.Record{ID: "trace"}, driver.SaveOptions{}); err == nil {
		t.Fatal("Save() error = nil, want uninitialized error")
	}
	if err := backend.Init(t.Context(), "tracing_documents", nil); err != nil {
		t.Fatalf("Init() error = %v", err)
	}
	if err := backend.Save(t.Context(), driver.Record{}, driver.SaveOptions{}); !errors.Is(err, driver.ErrInvalidField) {
