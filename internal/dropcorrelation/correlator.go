...
func (c *Correlator) ObserveKernel(event KernelDrop) bool { //nolint:gocritic // Copy isolates queued state from caller mutation.
...
func (c *Correlator) ObserveHardware(sample HardwareSample) []Incident { //nolint:gocritic // Copy isolates retained state from caller mutation.
...
func (c *Correlator) RecentSince(since time.Time) []Incident {
	...
	for index := range c.recent {
		incident := &c.recent[index]
