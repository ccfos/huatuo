...
func preflightELF(r io.ReaderAt, fileSize, limit uint64) error {
	...
	// Go debug/elf uses the raw program count, not section zero Info.
	if phnum == 0xffff {
		return fmt.Errorf("%w: extended ELF program counts are unsupported", errELFSymbolLimit)
	}
	...
	if shoff != 0 && (shnum == 0 || shstr == uint64(elf.SHN_XINDEX)) {
	...
	for _, table := range [][3]uint64{{shoff, shnum, shsize}, {phoff, phnum, phsize}} {
		...
		entryCost := size + elfHeaderMemoryAllowance
