...
func parseSession(msg *capnp.Message) (*Session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("transport: decode connect toolName: %w", err)
	}

	version, err := connect.Version()
	if err != nil {
		return nil, fmt.Errorf("transport: decode connect version: %w", err)
	}

	taskID, err := connect.TaskID()
	if err != nil {
		return nil, fmt.Errorf("transport: decode connect taskID: %w", err)
	}
