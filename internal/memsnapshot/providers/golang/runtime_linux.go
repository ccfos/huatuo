...
func inspectPID(ctx context.Context, procRoot string, pid int) (target, error) {
	...
	if info.elfType == elf.ET_DYN {
		mappings, mapsErr := memsnapshot.ReadProcMapsContext(ctx,
		...
		loadBias, err = memsnapshot.FindLoadBias(mappings, mapping, info.loadOffset,
			info.loadVaddr)
		if err != nil {
			return target{}, err
		}
...
func inspectExecutable(ctx context.Context, file *os.File) imageInfo {
	...
	elfFile, err := memsnapshot.ReadELFMetadata(ctx, file)
	if err != nil {
...
func readStartTimeTicks(path string) (uint64, error) {
	...
	return memsnapshot.ParseProcStatStartTime(data)
}
