...
func capture(ctx context.Context,
	...
	scanDeadline, hasDeadline := memsnapshot.DeadlineWithReserve(
		ctx, resultBuildReserve,
	)
	memory := processMemory{
		pid: readPID, ctx: ctx, deadline: scanDeadline, hasDeadline: hasDeadline,
	}
	if err := memsnapshot.ValidateIdentity(procRoot, identity); err != nil {
		return nil, err
	}
	...
	snapshot := &memsnapshot.Snapshot{RuntimeVersion: metadata.image.displayVersion()}
	...
	if len(aggregates) == 0 {
		...
		identityErr := memsnapshot.ValidateIdentity(procRoot, identity)
	...
	objects := make([]memsnapshot.ObjectAggregate, 0, len(aggregates))
...
func scanHumongous(memory processMemory, metadata *vmMeta, groups []regionGroup,
	...
	samples map[uint64]classSample, snapshot *memsnapshot.Snapshot,
) (int, error) {
	failedGroups := 0
		...
		if readErr == nil {
			...
			if !validKlass {
				...
				if klass == nil {
					...
					if readErr == nil && !metadata.cacheKlass(classes, klassAddress, klass) {
						break
					}
				}
	...
	reason := scanWindows(memory, regions, seed,
		...
		func(batch []sampleWindow) bool {
			...
			for _, sample := range batch {
				batchBytes = memsnapshot.SaturatingAdd(batchBytes, uint64(len(sample.raw)))
			}
			sampledBytes = memsnapshot.SaturatingAdd(sampledBytes, batchBytes)
			...
			for _, sample := range batch {
				statistics.ordinarySampledBytes = memsnapshot.SaturatingAdd(
...
func finishStatus(snapshot *memsnapshot.Snapshot, failedHumongousGroups int,
...
func appendPartial(snapshot *memsnapshot.Snapshot, reason string) {
