...
func TestIsCollectionDimensionLabel(t *testing.T) {
	for _, name := range CollectionDimensionLabelNames() {
		if !IsCollectionDimensionLabel(name) {
			t.Errorf("%q is not recognized as a collection label", name)
		}
	}
	if IsCollectionDimensionLabel("user_supplied") {
		t.Fatal("user_supplied was recognized as a collection label")
	}
}
