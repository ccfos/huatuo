...
- ACPI GHES
  ...
  | `data_len` | 原始错误数据载荷长度（字节数），与内核上报的 tracepoint `len` 字段一致。该值**不会被裁剪到捕获窗口**，因此对于较大的 CPER section（NVDIMM、CXL 及厂商自定义 section 通常达 1 KiB 以上），可能大于 `raw_data` 实际列出的字节数。 |
  | `raw_data` | 原始错误数据的十六进制转储（空格分隔字节）。代理只捕获记录中固定的 456 字节窗口（`DETAIL_INFO_SIZE_ACPI` = 512 − 56），更大的 section 会被截断到已捕获的字节，解码时安全降级而不会崩溃。用于深度诊断，需结合具体硬件厂商文档解析。 |
