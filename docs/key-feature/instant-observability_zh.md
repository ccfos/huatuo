...
当前支持 13 类事件的持续观测，覆盖 CPU 调度健康状态（sched_tick、softlockup、hungtask）、内存压力（memory_oom、memory_reclaim_events）、网络协议栈（dropwatch、tcp_retransmit、net_rx_latency、net_tx_latency、netdev_events、netdev_bonding_lacp、netdev_txqueue_timeout）以及硬件可靠性（ras）等方面。
