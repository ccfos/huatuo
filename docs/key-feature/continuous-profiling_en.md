...
downloads and installs versioned Memray runtimes under
`_output/tools/memray/runtimes`.
...
default and does not require `--tool-path`. The process running `profiler`
must have `gdb` in `PATH` and permission to attach to the target with
`ptrace` (normally root or `CAP_SYS_PTRACE`); seccomp and Yama `ptrace_scope`
...
Python memory profiling uses the prebuilt Memray bundle installed by
`make all`. It reports memory still live at the end of the profiling window,
rather than allocation throughput over time. Do not pass `--memory-mode` or
`--tool-path` for the normal bundled setup:
