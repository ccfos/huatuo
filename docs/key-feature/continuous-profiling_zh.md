...
原生远端 profile 使用现有 pprof 字段携带聚合窗口：`TimeNanos` 是窗口起点
（Unix 纳秒），`DurationNanos` 是实际经过的时长，而非配置中的 `--duration`
或 `--aggr-interval`。首窗从聚合流水线启动时开始，后续窗口从上一次成功的快照
切换边界开始。最终边界在流水线停止接收记录时固定，不等待队列排空或上传完成，
因此提前停止会产生实际长度的部分尾窗，而不是补成完整配置周期。

快照内容与窗口一起冻结，上传延迟及重试不会移动已冻结窗口。上传失败时，新的
活动窗口继续收集数据，可能超过 `--aggr-interval`；快照失败则保留原起点。
存储中的 `started_timestamp` 来自 profile 时间，`uploaded_timestamp` 仍是
独立的存储／上传时间。

这里的窗口是**聚合观测窗口**，不是精确的逐事件内核时间。采集器异步交付批量
数据，流水线启停也包围采集器的初始化和清理。因此窗口可能包含初始化／清理
耗时，或接收在聚合边界之前已采集的记录。本时间元数据不会恢复缺失批次、在
取消时额外排空内核事件，也不保证精确的事件时间归属。

原生 on-CPU、off-CPU、分配和锁 profile 使用区间时长。`physical_usage` 仍是
已观测分配／释放形成的净留存快照，时间取聚合截止点，duration 为零，并非完整
RSS 快照。Java/Python 时间元数据及本地 folded/SVG 输出不变。样本值及单位
不变：CPU 次数仅按采样周期缩放一次，off-CPU 纳秒和内存字节不会再按窗口时长
缩放。
