...
| `ktime_ns`               | uint64   | Kernel hook monotonic timestamp in nanoseconds, comparable only within the same host boot, not UTC time; absent in older events means unavailable, and zero is omitted |
