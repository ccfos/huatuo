...
#### 8.4 Network Transmit Latency Tracing

```bash
# TCP send-path latency thresholds are in milliseconds.
...
- **Sendmsg2Qdisc**: Threshold from `tcp_sendmsg` entry until the packet
  reaches `net_dev_queue`. Default: 50ms.
- **Qdisc2DevXmit**: Threshold for time queued between `net_dev_queue` and
  `net_dev_start_xmit`. Default: 10ms.
- **DevXmit2Nic**: Threshold for the device driver's transmit call between
  `net_dev_start_xmit` and `net_dev_xmit`. Default: 1ms.
- **ExcludedContainerQos**: Container QoS levels to skip, using the same
  case-insensitive values as receive latency tracing. Default: `[]`.
- **ExcludedHostNetnamespace**: Exclude packets in the host network namespace.
  Default: `true`.

The tracer exposes `huatuo_bamai_net_tx_latency_events_total`,
`huatuo_bamai_net_tx_latency_seconds_total`, and
`huatuo_bamai_net_tx_latency_last_seconds` Prometheus metrics with a `stage`
label.
...
#### 8.5 Network Device Event Monitoring
...
#### 8.7 TCP Retransmission Tracing ([EventTracing.TCPRetransmit])
...
#### 8.8 Hardware Error Event Tracing (EventTracing.Ras)
