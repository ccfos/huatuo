...
function wait_for_vm_ready() {
    ...
    for ((i=1; i<=timeout; i+=interval)); do
        if ssh "${SSH_OPTS[@]}" "root@${VM_NAME}" "uname -a" 2>/dev/null; then
            echo -e "✔ vm ${VM_NAME} network is ready, wait for mount /mnt/host..."
            if ssh "${SSH_OPTS[@]}" "root@${VM_NAME}" "ls -lah /mnt/host" 2>/dev/null; then
                return 0
            fi
        fi
...
function wait_for_k8s_ready() {
    ...
    for ((i=1; i<=timeout; i+=interval)); do
        if ssh "${SSH_OPTS[@]}" "root@${VM_NAME}" "kubectl get pods -A | grep coredns | grep -q Running"; then
            return 0
        fi
        echo -e "waiting for coredns to become ready... ${i}/${timeout}s"
        sleep $interval
    done

    echo -e "⚠️ coredns not ready after ${timeout}s"
