...
func TestBuildCreateProfilingJobRequest(t *testing.T) {
	tests := []struct {
		...
		{
			name: "lock profiling without target",
			req: v1.CreateProfilingJobRequest{
				ProfilingType:   "lock",
				Language:        "c",
				DurationSeconds: 30,
			},
			wantErr: "lock profiling requires exactly one pid or container_id",
		},
		{
			name: "container lock profiling with thread group",
			req: v1.CreateProfilingJobRequest{
				ProfilingType:   "lock",
				Language:        "c",
				ContainerID:     "container-2026",
		...
		{
			name: "lock profiling with invalid threshold",
			req: v1.CreateProfilingJobRequest{
				ProfilingType:     "lock",
				Language:          "c",
				PID:               42,
				LockWaitThreshold: "fast",
				DurationSeconds:   30,
			},
			wantErr: `invalid lock_wait_threshold "fast": time: invalid duration "fast"`,
		},
		{
			name: "lock profiling with zero threshold",
			req: v1.CreateProfilingJobRequest{
				ProfilingType:     "lock",
				Language:          "c",
				PID:               42,
			...
			wantErr: "lock_wait_threshold must be between 1us and 1h",
		},
		{
