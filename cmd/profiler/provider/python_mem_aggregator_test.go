...
func TestPythonMemoryAggregatorLabelsFlameGraphValuesAsBytes(t *testing.T) {
	pctx := &pcontext.ProfilerContext{
		Ctx:          context.Background(),
		OutputFormat: output.FormatFlameGraph,
	}
	aggr, err := newPythonMemoryAggregator(pctx)
	if err != nil {
		t.Fatal(err)
	}
	aggr.Aggregate(profiler.SampleOutput{Output: "process 123:python;foo 1024\n"})

	var rendered bytes.Buffer
	if err := aggr.OutputFormatter().Write(&rendered); err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(rendered.String(), "foo (1024 bytes, 100.00%)") {
		t.Fatalf("flame graph does not label retained memory as bytes: %s", rendered.String())
	}
}
