...
func TestValidateProfilerFlagCompatibility(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "Java follow forks",
			language:  "java",
			typ:       "cpu",
			args:      []string{"--follow-forks", "--pid", "123"},
			wantError: "--follow-forks is supported only by native profilers",
		},
		{
			name:      "native follow forks without PID",
			language:  "c",
			typ:       "cpu",
			args:      []string{"--follow-forks", "--container-id", "abc"},
			wantError: "--follow-forks requires --pid",
		},
		{
			name:      "fork limit without tracking",
			language:  "c",
			typ:       "cpu",
			args:      []string{"--fork-rate", "5"},
			wantError: "fork limit flags require --follow-forks",
		},
		{
			name:      "fork maximum zero",
			language:  "c",
			typ:       "cpu",
			args:      []string{"--follow-forks", "--pid", "123", "--fork-max-procs", "0"},
			wantError: "--fork-max-procs must be between 1 and 65536",
		},
		{
			name:     "native follow forks",
			language: "c",
			typ:      "cpu",
			args:     []string{"--follow-forks", "--pid", "123", "--fork-rate", "0"},
		},
		{
			name:      "Java scope",
			language:  "java",
