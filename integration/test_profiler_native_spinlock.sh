...
WORK_DIR=$(mktemp -d \
	"${HUATUO_BAMAI_TEST_TMPDIR}/profiler-spinlock.XXXXXX")
MODULE_DIR="${WORK_DIR}/module"
MODULE_NAME="huatuo_spinlockprof_test"
DEVICE_PATH="/dev/huatuo_spinlockprof_fixture-${BASHPID}"
...
if [[ ${#folded_files[@]} -eq 0 ]]; then
	grep -q "attached via contention tracepoints" "${WORK_DIR}/profiler.out" \
		&& skip "kernel contention tracepoints produced no spinlock samples"
