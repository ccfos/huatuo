...
// Use the same registered constructors and Start entry point as the daemon.
func liveTracer(t *testing.T, name string) interface {
	...
	attr := registry[name]
	if attr == nil {
		t.Fatalf("missing registered tracer %s", name)
	}
	tracer, ok := attr.TracingData.(interface {
		Start(ctx context.Context) error
	})
	if !ok {
		t.Fatalf("tracer %s has no Start entry point", name)
	}
	return tracer
}

func liveStore(t *testing.T, name string) func() ([]*tracingstore.Document, error) {
	t.Helper()
	dir := t.TempDir()
...
func TestDloadHostLiveTrigger(t *testing.T) {
	...
	if err != nil || pid <= 1 {
		t.Skip("set HUATUO_DLOAD_WORKER_PID to a bounded D-state worker; keep at least two D-state threads alive for 90s")
	}
	initDloadLiveBPF(t)
	store := liveStore(t, "dload")
	d := liveTracer(t, "dload")
