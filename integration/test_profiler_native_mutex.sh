...
WORK_DIR=$(mktemp -d \
	"${HUATUO_BAMAI_TEST_TMPDIR}/profiler-mutex.XXXXXX")
MODULE_DIR="${WORK_DIR}/module"
MODULE_NAME="huatuo_mutexprof_test"
DEVICE_PATH="/dev/huatuo_mutexprof_fixture-${BASHPID}"
...
if [[ ${#folded_files[@]} -eq 0 ]]; then
	grep -q "attached via contention tracepoints" "${WORK_DIR}/profiler.out" \
		&& skip "kernel contention tracepoints produced no mutex samples"
	fatal "no folded output produced"
fi
grep -q "lock type: mutex" "${folded_files[@]}" \
	|| fatal "mutex type frame not found"
