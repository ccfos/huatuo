...
func (runner *schedBlameRunner) evaluate(now time.Time) error {
	superMonitorSamples := runner.sampleHighlightedSuperMonitor()
...
func (runner *schedBlameRunner) sampleHighlightedSuperMonitor() map[uint64]schedBlameSuperMonitorSample {
	if !runner.config.highlightConfigured() {
		runner.superMonitorSampler.retain(nil)
		return nil
	}
	activeContainerIDs := make(map[string]struct{})
	samples := make(map[uint64]schedBlameSuperMonitorSample)
	target := runner.state.targets[schedBlameHighlightIndex]
	if !target.active() {
		runner.superMonitorSampler.retain(activeContainerIDs)
		return samples
	}
	activeContainerIDs[target.containerID] = struct{}{}
	samples[target.cgid] = runner.superMonitorSampler.sample(
		target.containerID,
		target.cgroupPath,
		target.cgid,
	)
