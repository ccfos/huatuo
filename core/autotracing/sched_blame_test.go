			...
			samples: []schedBlameExternalRatioDebugSample{
				{
					...
					throttledTimeNs:      25,
					emitted:              true,
					superMonitorSample: schedBlameSuperMonitorSample{
						valid:              true,
						waitrate:           0.4375,
...
func TestSchedBlameUploaderCancelsBlockedWriteAfterDrainTimeout(t *testing.T) {
	...
	case <-time.After(time.Second):
		t.Fatal("uploader did not stop after its writer observed cancellation")
	}
...
func TestSchedBlameUploaderCloseIsBoundedWhenWriterIgnoresCancellation(t *testing.T) {
	started := make(chan struct{})
	...
	case <-time.After(time.Second):
		close(release)
		t.Fatal("uploader close waited for a writer that ignored cancellation")
	}
	...
	case <-uploader.done:
	case <-time.After(time.Second):
		t.Fatal("uploader did not stop after the writer returned")
	}
}
