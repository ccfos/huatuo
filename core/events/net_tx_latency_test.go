...
func TestNewNetTxTracingData(t *testing.T) {
	...
	event.LatStage = 2
	event.Latency = uint64(2500 * time.Microsecond)
	event.PktLen = 1500
	event.NetNSInum = 123
	event.NetNSCookie = 456
	...
	if data.LatStage != "TX_STAGE_DEV_NIC" || data.LatMs != 2.5 {
		t.Fatalf("latency data = %q/%v, want TX_STAGE_DEV_NIC/2.5", data.LatStage, data.LatMs)
	}
	if data.NetdevName != "eth0" || data.PktLen != 1500 {
		t.Fatalf("packet data = %q/%d, want eth0/1500", data.NetdevName, data.PktLen)
	}
...
func BenchmarkNewNetTxTracingData(b *testing.B) {
	event := abi.NetRXLatencyEvent{
		LatStage: 1,
		Latency:  uint64(2 * time.Millisecond),
		PktLen:   1500,
	}
	b.ReportAllocs()
	for b.Loop() {
