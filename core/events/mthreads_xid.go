...
const (
	...
	// 256 bytes, so this covers up to 32 events queued at once.
	...
	// eviction. See mthreadsXidCollector.seen for the full trade-off.
...
func (d musaDevice) eventReportPath() string {
	return fmt.Sprintf("/proc/driver/musa/gpu%02x/event_report", d.phyIdx)
}
...
func newMthreadsXid() (*tracing.EventTracingAttr, error) {
	devices, err := scanMusaDevices("/proc/driver/musa")
	if err != nil || len(devices) == 0 {
		// No MUSA hardware (or driver half-loaded) is normal on most
		// hosts; stay at Debug so non-MUSA deployments don't spam logs.
		log.Debugf("mthreads_xid: no MUSA devices under /proc/driver/musa (err=%v, n=%d), tracer inactive",
			err, len(devices))
	...
	return &tracing.EventTracingAttr{
		TracingData: &mthreadsXidCollector{
			...
			data: []*metric.Data{
				metric.NewCounterData("total", 0, "mthreads_xid counter", nil),
			},
		},
		Interval: 10,
...
func (c *mthreadsXidCollector) Update() ([]*metric.Data, error) {
	c.data[0].Value = float64(atomic.LoadInt64(&mthreadsXidCounter))
	return c.data, nil
}
...
func (c *mthreadsXidCollector) Start(ctx context.Context) error {
	...
	threshold := severityThreshold(configSnapshot().MthreadsXid.ReportLevel)
	...
	for {
		...
		for _, fd := range fds {
			c.drainDevice(fd, registeredAt, threshold, buf)
		}
	}
}
...
// severity filter.
func (c *mthreadsXidCollector) drainDevice(fd int, registeredAt time.Time, minSeverity uint8, buf []byte) {
...
func scanMusaDevices(root string) ([]musaDevice, error) {
	...
	for _, e := range entries {
		...
		devname, err := os.ReadFile(filepath.Join(root, name, "devname"))
		if err != nil {
			continue
		}
...
func severityThreshold(level MthreadsXidLevel) uint8 {
	...
	default:
		// Unknown / unset → permissive (report all severities).
		return 0
	}
}
