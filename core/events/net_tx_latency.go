	...
	for {
		...
		default:
			...
			containerID, ok := filterNetLatencyEvent(
				...
				event.NetNSCookie,
				hostNetNSInum,
				cfg.NetTxLatency.ExcludedHostNetnamespace,
				cfg.NetTxLatency.ExcludedContainerQos,
				"net_tx_latency",
			)
			if !ok {
				continue
			...
			c.recordMetric(stage, event.Latency)
...
func newNetTxTracingData(event *abi.NetRXLatencyEvent, threshold uint64) *NetTracingData {
	return &NetTracingData{
		...
		LatStage:           netTxLatencyStageNames[event.LatStage],
		LatMs:              float64(event.Latency) / float64(time.Millisecond),
		LatThresholds:      threshold,
		NetdevName:         bytesutil.ToStr(event.NetdevName[:]),
		NetNamespaceInum:   event.NetNSInum,
		NetNamespaceCookie: event.NetNSCookie,
