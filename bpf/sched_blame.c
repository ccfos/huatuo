	...
	void *ctx, u64 cpu_index, u64 now)
{
	...
	// A nonzero first timestamp implies that the per-CPU batch is nonempty.
...
int on_sched_switch(struct bpf_raw_tracepoint_args *ctx)
{
	...
	if (next_tg && next_css_id < MAX_CSS_IDS) {
		// A directly throttled cgroup cannot be selected as next. Apply next
		// second so a same-cgroup switch gets its final state.
...
int on_sched_migrate_task(struct bpf_raw_tracepoint_args *ctx)
{
	...
	if (cpu_is_running_css_id(src_cpu_index, css_index)) {
		// The same-cgroup current task remains counted, so subtraction is safe.
