...
extern "C" __attribute__((visibility("default"))) int memray_spawn_client(int port);
...
connect_client(const uint16_t port)
{
    ...
    // Connect to the localhost control server started by the CLI
    const char* host = "127.0.0.1";
    const int rv = ::getaddrinfo(host, port_str.c_str(), &hints, &all_addresses);
    if (rv != 0) {
...
// Callback executed by the Python interpreter main thread that runs the
// memray client directly without spawning a new pthread.
static int
memray_pending_cb_direct(void* arg)
{
    int port = static_cast<int>(reinterpret_cast<uintptr_t>(arg));
    memray::run_client(static_cast<uint16_t>(port));
    return 0;
}
...
memray_schedule_client_direct(int port)
{
    ...
    return Py_AddPendingCall(&memray::memray_pending_cb_direct, (void*)(uintptr_t)port);
}
